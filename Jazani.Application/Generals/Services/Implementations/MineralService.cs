...
namespace Jazani.Application.Generals.Services.Implementations
{
    public class MineralService : IMineralService
    {
        ...
        public async Task<MineralDto> CreateAsync(MineralSaveDto savaDto)
        {
            Mineral mineral = _mapper.Map<Mineral>(savaDto);
        ...
        public async Task<MineralDto> EditAsync(int id, MineralSaveDto savaDto)
        {
            Mineral mineral = await _mineralRepository.FindByIdAsync(id);

            _mapper.Map<MineralSaveDto, Mineral>(savaDto, mineral);
